...
router.post("/create-checkout-session", async (req, res) => {
	try {
		...
		// 1) create payment record (email = null, will be set by Stripe webhook)
		...
		// 2b) broadcast "reserved" to WS clients
		const hub = getSeatsHub();
		if (hub) {
			for (const seat_uid of seat_uids) {
				hub.broadcastSeatUpdate(showtime_uid, seat_uid, "reserved");
			}
		}
