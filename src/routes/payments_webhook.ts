...
const webhookHandler = async (req: Request, res: Response) => {
	...
	if (event.type === "checkout.session.completed") {
		...
		try {
			...
			// ===== WS BROADCAST: status "paid" =====
			const hub = getSeatsHub();
			if (hub && showtime_uid !== "unknown" && seat_uids !== "unknown") {
				const seats = String(seat_uids).split(",");
				for (const seat of seats) {
					hub.broadcastSeatUpdate(showtime_uid, seat, "paid");
				}
				console.log("WS broadcast sent for seats:", seats);
			}
