...
adminRouter.get("/cities/:city_uid", authenticate, async (req, res) => {
  ...
  const { rows } = await pool.query("SELECT * FROM city WHERE uid = $1", [
    city_uid,
  ]);
...
adminRouter.get("/cinemas/:cinema_uid", authenticate, async (req, res) => {
  ...
  const { rows } = await pool.query("SELECT * FROM cinema WHERE uid = $1", [
    cinema_uid,
  ]);
  if (rows.length === 0)
    return res.status(404).json({ msg: "cinema not found" });
  res.json(rows[0]);
});
...
adminRouter.get("/hall/:hall_uid", authenticate, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query("SELECT * FROM hall WHERE uid = $1", [
      hall_uid,
    ]);
    if (rows.length === 0)
      return res.status(404).json({ msg: "hall not found" });
...
adminRouter.delete(
  ...
  async (req, res) => {
    ...
    try {
      const { rowCount } = await pool.query("DELETE FROM hall WHERE uid = $1", [
        hall_uid,
      ]);
      if (rowCount === 0)
        return res.status(404).json({ msg: "hall not found" });
